...
GROUP_A: list[str] = ["TIDE_SPOT", "WX_SPOT", "LHR_COUNT", "LON_ETF"]
...
COMP_START = datetime(2026, 2, 28, 14, 30, tzinfo=UTC)  # Fri Feb 28 2:30 PM UTC
SETTLEMENT_TIME = datetime(2026, 3, 1, 12, 0, tzinfo=UTC)  # Sat March 1 12:00 PM UTC
# Settlement counts flights over 24h ending at SETTLEMENT_TIME (not from COMP_START)
SETTLEMENT_WINDOW_START = datetime(2026, 2, 28, 12, 0, tzinfo=UTC)  # 24h before settlement

# Arbitrage
MIN_ARB_EDGE = 1.5  # minimum tick edge to trigger arbitrage
MIN_BOOK_DEPTH = 2  # reduced to trade in thinner books

# Scratch
MAX_SCRATCH_COST = 500.0  # maximum P&L cost of a scratch reversal (skip if too expensive)
...
# Stop-loss
STOP_LOSS_PNL = 250_000.0  # halt all trading if total PnL drops below this level
